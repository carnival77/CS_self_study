...
<details>
  ...
  <p>먼저 클라이언트가 연결을 종료하겠다는 FIN 플래그를 전송하고, 서버는 확인 메세지 ACK을 보내고 남은 전송 데이터를 이어서 보냅니다. </p>
<p>그리고 통신이 끝나면 연결 종료를 수락한다는 의미로 FIN을 클라이언트에게 보내고, 클라이언트는 이를 확인했다고 ACK을 보내고, 서버는 이 값을 받고 연결을 해제합니다.</p>
</details>
