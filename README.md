...
<details>
  ...
  <p>장점으로는 빠른 검색 성능을 들 수 있습니다.</p>
  <p>단점으로는 인덱스를 구성하는 비용 즉, 추가, 수정, 삭제 연산시에 인덱스를 형성하기 위한 추가적인 연산이 수행됩니다.</p>
  <p>이 세가지 연산은 데이터가 바뀌었을 때 인덱스 테이블의 수정도 함께 필요하기 때문에 무분별하게 인덱스를 사용하면 성능저하를 유발할 수 있습니다.</p>
...
<details>
  ...
  <p>트랜잭션이란 데이터베이스의 상태를 변화시키기 위해 수행하는 작업의 완전성 을 보장해주는 작업 단위</p>
...
<details>
  ...
  <p>
    <ul>
      ...
      <li>Isolation(고립성): 각각의 트랜잭션은 서로 간섭없이 독립적으로 수행되어야 한다. 즉, 둘 이상의 트랜잭션이 동시에 병행 실행되고 있을 때, 한 트랜잭션 작업에 다른 트랜잭션의 연산 작업이 끼어들지 못하도록 보장하며 트랜잭션의 밖에 있는 어떤 연산도 중간 단계의 데이터를 볼 수 없음</li>
      <li>Durability(지속성): 트랜잭션이 커밋된 이후에는 영구적으로 데이터베이스에 작업의 결과가 저장되어야 한다. 즉, 시스템 오류가 발생하더라도 커밋된 상태로 유지되는 것을 보장해야 합니다. 많은 DB들은 트랜잭션 조작은 로그로 기록하고 시스템에 이상이 생기면 복원하는 식으로 지속성을 실현
        </li>
    </ul>
  </p>
</details>
